...
if [ $# -ne 5 ]; then
    ...
    Rscript index.r $1 $2
    mkdir $5
    cat ../realign.raw.count | awk 'NR==FNR&&NR>=1{a[$1]=0}NR>FNR{b=$1;for(i=2;i<=NF;i++){if((i-1) in a){b=b"\t"$i}};print b}' $2 - >$5/expression_raw_count
    ...
    awk 'BEGIN{FS="\t";OFS="\t"}NR==FNR{a[$1]=1}NR>FNR{if($4 in a){start=$3;$3=$2;$2=start}; print $0}' <(zcat /zs32/home/yjiang/database/gencode.v19.annotation.bed.gz |awk 'BEGIN{FS="[\t.]"}$5=="gene"{a[$7]=$6}END{for(i in a){if(a[i]=="-") print i}}') <(awk 'BEGIN{FS="\t";OFS="\t"}NR==FNR&&$5=="gene"{split($8,s,".");a[s[1]]=$1"\t"$2"\t"$3}NR>FNR&&FNR==1{print "#chr\tstart\tend\tgene"$0}NR>FNR&&FNR>1&&($1 in a){print a[$1],$0}' <(zcat /zs32/home/yjiang/database/gencode.v19.annotation.bed.gz) $5/log2cpm.fgene.fsample.qn) |sed -e's/^chr//'>$5/log2cpm.fgene.fsample.qn.bed
    ...
    parallel -j 10 /zs32/home/shliu/softwares/QTLtools/QTLtools_1.0_CentOS6.8_x86_64 cis --vcf $5/genotypes.all.chr{}.vcf.gz --bed $5/phenotype.chr{}.bed.gz --region {} --cov $5/Covariant.txt --out $5/eqtl.permute.chr{} --permute 1000 ::: {1..22} X
    parallel -j 10 /zs32/home/shliu/softwares/QTLtools/QTLtools_1.0_CentOS6.8_x86_64 cis --vcf $5/genotypes.all.chr{}.vcf.gz --bed $5/phenotype.chr{}.bed.gz --region {} --cov $5/Covariant.txt --out $5/eqtl.nopermute.chr{} --nominal 1 ::: {1..22} X
    cat $5/eqtl.permute.chr* | gzip -c > $5/permutations_all.txt.gz
    Rscript ~/softwares/QTLtools/script/runFDR_cis.R $5/permutations_all.txt.gz $4 $5/permutations_all
    parallel -j 10 /zs32/home/shliu/softwares/QTLtools/QTLtools_1.0_CentOS6.8_x86_64 cis --vcf $5/genotypes.all.chr{}.vcf.gz --bed $5/phenotype.chr{}.bed.gz --region {} --cov $5/Covariant.txt --out $5/eqtl.conditional.chr{} --mapping $5/permutations_all.thresholds.txt ::: {1..22} X
    cd $5
    Rscript ../FDR.r
